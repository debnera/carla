...
#include "Carla.h"
#include "MyWheeledVehicleAIController.h"
...
static bool MyRayTrace(const AActor &Actor, const FVector &Start, const FVector &End) {
  FHitResult OutHit;
...
static bool MyIsThereAnObstacleAhead(
    ...
    const FVector &Direction)
{
  ...
  return
      MyRayTrace(Vehicle, StartCenter, EndCenter) ||
      MyRayTrace(Vehicle, StartRight, EndRight) ||
      MyRayTrace(Vehicle, StartLeft, EndLeft);
}

template <typename T>
static void MyClearQueue(std::queue<T> &Queue)
{
...
AMyWheeledVehicleAIController::AMyWheeledVehicleAIController(const FObjectInitializer& ObjectInitializer) :
...
AMyWheeledVehicleAIController::~AMyWheeledVehicleAIController() {}

// =============================================================================
// -- APlayerController --------------------------------------------------------
// =============================================================================

void AMyWheeledVehicleAIController::Possess(APawn *aPawn)
{
  Super::Possess(aPawn);
...
void AMyWheeledVehicleAIController::UnPossess()
{
  Super::UnPossess();
...
void AMyWheeledVehicleAIController::Tick(const float DeltaTime)
{
...
void AMyWheeledVehicleAIController::ConfigureAutopilot(const bool Enable)
{
...
void AMyWheeledVehicleAIController::SetFixedRoute(
...
void AMyWheeledVehicleAIController::TickAutopilotController()
{
#if WITH_EDITOR
  ...
  if (TrafficLightState != ETrafficLightState::Green) {
    ...
    Throttle = Stop(Speed);
  } else if (MyIsThereAnObstacleAhead(*Vehicle, Speed, Direction)) {
...
float AMyWheeledVehicleAIController::GoToNextTargetLocation(FVector &Direction)
{
...
float AMyWheeledVehicleAIController::CalcStreeringValue(FVector &direction)
{
  float steering = 0;
...
float AMyWheeledVehicleAIController::Stop(const float Speed) {
  return (Speed >= 1.0f ? -Speed / SpeedLimit : 0.0f);
}

float AMyWheeledVehicleAIController::Move(const float Speed) {
  if (Speed >= SpeedLimit) {
    return Stop(Speed);
